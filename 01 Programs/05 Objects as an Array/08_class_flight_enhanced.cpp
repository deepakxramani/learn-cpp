...
class Flight
{
    ...
    int searchbySAND(string,string,int,int);
    
    
};
int Flight::searchbySAND(string s,string d, int min, int max)
{
...
int main(){
    Flight F[3];
    int i,min,max,ch,found,count=0;
    string id,s,d;
    for(i=0;i<=2;i++)
    {
        F[i].getFlights();
    ...
    do{
        cout<<"\nMain Menu\n1]Show All Flights\n2]Search by ID\n3]Search by Source,Destination & Fare\n4]Exit";
        cout<<"\nEnter Your Choice: ";
        cin>>ch;
            switch(ch)
            {
                ...
                case 3:
                    ...
                    cin>>d;
                    cout<<"Enter Minimum Fare"<<endl;
                    cin>>min;
                    cout<<"Enter Maximum Fare"<<endl;
                    cin>>max;
                    cout<<"Company Name"<<"\t\t"<<"Flight ID"<<"\t\t"<<"Source"<<"\t\t"<<"Destination"<<"\t\t"<<"Days"<<"\t\t"<<"Fare"<<endl;
                    found=0;
                    count=0;
                    for(i=0;i<=2;i++)
                    {
                        found=F[i].searchbySAND(s,d,min,max);
                        if(found)
                        {count++;}
                    }
                    if(count==0)
